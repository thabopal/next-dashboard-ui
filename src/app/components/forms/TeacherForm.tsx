        ...
        <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            ...
            <div className="flex justify-between flex-wrap gap-4">
                ...
                <div className="flex flex-col gap-2 w-full md:w-1/4 justify-center">
                    ...
                    <select multiple className="ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("subjects")} defaultValue={data?.gender}>
                        {subjects.map((subject: { id: number; name: string }) => (
                            <option key={subject.id} value={subject.id} selected={data && subject.id === data.id}>{subject.name}</option>
                        ))}
                    </select>
