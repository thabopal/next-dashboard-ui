...
async function main() {
  ...
  // GRADE
  const gradeIds = []; // To store the grade IDs
  for (let i = 8; i <= 12; i++) {
    ...
    gradeIds.push(grade.id); // Store the generated grade ID
  }

  // CLASS
  for (let i = 8; i <= 12; i++) {
    const gradeId = gradeIds[i - 8]; // Retrieve the corresponding gradeId from the array
